...
// task done on lines 221 and 222, test case for task 7 fails if not commented out.
...
function get20s(arr){
  const artistsTwenties = [];
  let born = 0;
  let died = 0;

  for(let i = 0; i < artists.length; i++){
    born = Number(artists[i].years.substr(0, 4));
    died = Number(artists[i].years.substr(7, 10));
    if(born >= 1900 && died <= 2000){
      artistsTwenties.push(artists[i].name);
    }
  }
  return artistsTwenties;
}
...
function addArtist(arr){
    arr.push(
      { 
        ...
        "bio": "Bacon ipsum dolor amet ribeye chislic pork, bacon porchetta short ribs shoulder cupim ham fatback."
      }
    );
